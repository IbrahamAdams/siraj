...
class _ItemEntryViewState extends State<ItemEntryView> {
  ...
  Widget build(BuildContext context) {
    ...
    dynamic updateImages(List<XFile> resultList, int index, int currentIndex) {
      // if (index == -1) {
...
class ImageUploadHorizontalListState extends State<ImageUploadHorizontalList> {
  ...
  Future<void> loadPickMultiImage(int index) async {
    ...
    try {
      ...
      // التأكد من الصور المختارة
      if (resultList != null && resultList.isNotEmpty) {
        for (var image in resultList) {
          print('Selected Image: ${image.path}');
          // يمكنك تنفيذ العمليات المطلوبة هنا
        }
      } else {
  ...
  Future<void> loadSingleImage(int index) async {
    ...
    try {
      // السماح للمستخدم بالتقاط صورة أو اختيارها
      pickedImage = await _picker.pickImage(
        source: ImageSource.gallery, // يمكنك استبدالها بـ ImageSource.camera
      );
    } catch (e) {
    ...
    // التحقق من نوع الملف إذا كنت بحاجة لذلك
    if (pickedImage.path.contains('.webp')) {
      showDialog<dynamic>(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: const Text("Error"),
            content: const Text("WebP images are not supported."),
            actions: [
              TextButton(
    ...
    // تنفيذ أي منطق بعد تحميل الصورة
    print('Selected Image Path: ${pickedImage.path}');

    // إذا كان لديك دالة لتحديث الصور، استخدمها هنا
    widget.updateImages!(pickedImage, index, index);
  }
