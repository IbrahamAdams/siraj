  ...
  Widget build(BuildContext context) {
    ...
    _chatRef.child(otherUserId!).onValue.listen((event) {
      if (event.snapshot.value == null) {
        ...
        final String? _receiverId = event.snapshot.value as String?;
    ...
    if (widget.psValueHolder.loginUserId == widget.messageObj.sendByUserId) {
      ...
      if (widget.messageObj.type == PsConst.CHAT_TYPE_TEXT) {
        if (widget.messageObj.offerStatus == PsConst.CHAT_STATUS_OFFER) {
          ...
          if (widget.index == 0 &&
              widget.chatHistoryProvider.chatHistory.data != null) {
            widget
                  ...
                  Text(
                    widget.itemDetail!.price != '0'
                        ? Utils.getChatPriceFormat(widget.messageObj.message!,
                            widget.psValueHolder!.priceFormat!)
    ...
    try {
      Future<void> pickImages() async {
        ...
        try {
          ...
          // التأكد من وجود الصور المختارة
          if (resultList != null && resultList.isNotEmpty) {
            for (var image in resultList) {
              print('Image selected: ${image.path}');
              // يمكنك تنفيذ العملية المطلوبة على الصور هنا
            }
          } else {
